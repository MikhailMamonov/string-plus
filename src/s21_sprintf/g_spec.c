...
char *g_spec(char *buf, long double val, formatSpec spec, int *len, char *start) {
  ...
  // 2. Делаем предварительный расчет экспоненты, чтобы понять, до какого знака округлять
  ...
  // Если число уйдет в %e, округляем до (prec - 1) знака относительно экспоненты.
  // Если в %f, то до (prec - 1 - temp_exponent) знаков.
  int round_prec = prec - 1 - temp_exponent;
  if (temp_exponent < -4 || temp_exponent >= prec) {
      round_prec = prec - 1;
      // Если число мелкое (например, 0.000095), сдвигаем округление под его масштаб
      if (temp_exponent < 0) {
  ...
  if (round_prec >= 0 && round_prec < 30) {
      ...
      if (is_negative) rounded_val = -rounded_val;
  }

  // 4. Считаем ФИНАЛЬНУЮ экспоненту по уже округленному числу
  int exponent = (rounded_val == 0.0l) ? 0 : (int)floorl(log10l(fabsl(rounded_val)));
...
long double round_banking(long double val, int precision) {
    ...
    if (fabsl(fabsl(fractional) - 0.5l) < epsilon) {
        // Если целая часть чётная — отбрасываем половинку (округляем вниз по модулю)
