...
char *handle_width(char *buf, int length, formatSpec spec) {
  ...
  if (spec.width > length) {
    ...
    if (spec.left_align) {
        ...
        char pad_char = ' ';
        if (spec.zero_padding && (spec.precision < 0 || s21_strchr("diouxX", spec.specifier) == s21_NULL) && strstr(buf - length, "NAN") == NULL && strstr(buf - length, "nan") == NULL && strstr(buf - length, "inf") == NULL && strstr(buf - length, "INF") == NULL) {
            pad_char = '0';
        }
        int minus_zero = 0;
        ...
        int plus_zero = 0;
        if (!minus_zero && *(buf - length) == '+' && pad_char == '0' && s21_strchr("difeEgG", spec.specifier) != s21_NULL) {
            plus_zero = 1;
        }
        int space_zero = 0;
        if (!minus_zero && !plus_zero && spec.space_sign && pad_char == '0' && s21_strchr("difeEgG", spec.specifier) != s21_NULL) {
            space_zero = 1;
        }
        int hex_zero = 0;
        if (s21_strchr("xX", spec.specifier) != s21_NULL && pad_char == '0' && spec.alt_format) {
            hex_zero = 1;
        }
        for (int i = 0; i < length; i++) {
            if ((minus_zero || plus_zero || space_zero) && i == length - 1) *to-- = '0';
            else if (hex_zero && *from == spec.specifier) *to-- = '0';
            else *to-- = *from--;
        }
        
        for (int i = 0; i < spaces_to_add; i++) {
            if (minus_zero && i == spaces_to_add - 1) *to-- = '-';
            else if (plus_zero && i == spaces_to_add - 1) *to-- = '+';
            else if (space_zero && i == spaces_to_add - 1) *to-- = ' ';
            else if (hex_zero && i == spaces_to_add - 2) *to-- = spec.specifier;
            else {
                *to-- = pad_char;
            }
        }
