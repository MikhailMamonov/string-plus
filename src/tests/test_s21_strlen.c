...
STRLEN_TEST_CASES(strlen_empty,
                  {"", 0, "empty_string"}
)

STRLEN_TEST_CASES(strlen_spaces,
                  {" ",        1, "single_space"},
                  {"   ",      3, "three_spaces"},
                  {"a b c",    5, "with_spaces"},
                  {"  abc  ",  7, "padded_spaces"}
...
STRLEN_TEST_CASES(strlen_special_chars,
                  {"line\n",      5,  "newline"},
                  {"tab\there",   8,  "tab_inside"},
                  {"path/to/x",   9,  "with_slashes"},
                  {"\"quoted\"",  8,  "quotes"},
                  {"!@#$%^&*()",  10, "punctuation"}
)

STRLEN_TEST_CASES(strlen_numbers,
                  {"42",          2, "two_digits"},
                  {"1234567890",  10, "ten_digits"},
                  {"0",           1, "zero"}
)

STRLEN_TEST_CASES(strlen_long_strings,
                  {"The quick brown fox jumps over the lazy dog", 43, "pangram"},
                  {"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", 50, "fifty_a"},
                  {"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", 100, "hundred_a"}
)
