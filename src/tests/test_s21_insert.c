...
static void run_insert_test(insertParams *params) {
    char *result = (char *)s21_insert(params->src, params->str,
                                      params->start_index);
...
INSERT_TEST_CASES(
    insert_basic,
    {"Hello world", ", beautiful", 5,  "Hello, beautiful world", "insert into middle"},
    {"world",       "Hello, ",     0,  "Hello, world",           "insert at start"},
    {"Hello",       " world",      5,  "Hello world",            "insert at end (index == len)"},
...
INSERT_TEST_CASES(
    insert_boundary,
    {"abc",   "X",    0, "Xabc",   "index 0"},
    {"abc",   "X",    3, "abcX",   "index == strlen"},
    {"a",     "BC",   0, "BCa",    "insert into single-char string at start"},
    {"a",     "BC",   1, "aBC",    "insert into single-char string at end"})

// Спецсимволы и пробелы
INSERT_TEST_CASES(
    insert_special_chars,
    {"line1\nline2",  "MID-", 6, "line1\nMID-line2", "insert after newline"},
    {"a\tb",          "X",    2, "a\tXb",            "insert after tab"},
    {"Hello, World!", "C ",   7, "Hello, C World!",  "insert with punctuation"},
    {"123",           "456",  1, "1456" "23",        "insert digits"})
...
INSERT_TEST_CASES(
    ...
    {"hello",  "X",       100,s21_NULL, "index far beyond strlen returns NULL"},
