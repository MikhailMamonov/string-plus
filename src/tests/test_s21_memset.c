...
MEMSET_TEST_CASES(string_tests, {"apple", 'p', 3, 6}, {"verter", 'a', 4, 7},
                  {"alll", 'l', 1, 5},
...
MEMSET_TEST_CASES(int_tests,
                  {(int[]){1, 2, 3, 4, 5}, '\0', 4 * sizeof(int) + 1,
                   5 * sizeof(int)},
                  {(int[]){100, 200, 300}, 'b', 3 * sizeof(int),
                   3 * sizeof(int)},
                  {(int[]){-1, -2, -3}, 0, 3 * sizeof(int), 3 * sizeof(int)})

// 3. Пограничные и экстремальные случаи
MEMSET_TEST_CASES(edge_tests, {"apple", -1, 3, 6}, {"verter", 257, 4, 7},
                  {"alll", 'l', 0, 5})
