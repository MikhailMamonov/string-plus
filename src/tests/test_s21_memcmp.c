...
static void run_memcmp_test(const memcmpParams *p) {
  ...
  ck_assert_msg(sign(expected) == sign(actual),
                ...
                p->test_name ? p->test_name : "no_name", actual, expected,
                (size_t)p->n);
}
